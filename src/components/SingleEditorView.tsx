...
import { getMarkknifeSlashMenuItems } from './markknifeEditorFormattingConfig'
import {
  MarkknifeFormattingToolbar,
  MarkknifeFormattingToolbarController,
} from './markknifeEditorFormatting'
import { MarkknifeSideMenu } from './markknifeBlockNoteSideMenu'
...
function MarkknifeOpenLinkButton({
  url,
  vaultPath,
...
function MarkknifeLinkToolbar({ vaultPath, ...props }: LinkToolbarProps & { vaultPath?: string }) {
  return (
    <LinkToolbar {...props}>
      <EditLinkButton
        url={props.url}
      ...
      />
      <MarkknifeOpenLinkButton url={props.url} vaultPath={vaultPath} />
      <DeleteLinkButton
        range={props.range}
        setToolbarOpen={props.setToolbarOpen}
      />
...
function handleSelectedEditorCopy(
  ...
  // 优先用 BlockNote 选区 HTML 以保留富文本格式,失败时回退到 DOM 提取的 HTML
  ...
  const getSlashMenuItems = useCallback(async (query: string) => {
    try {
      return guardSuggestionMenuItems(
        await Promise.resolve(getMarkknifeSlashMenuItems(editor, query, {
...
export function SingleEditorView({ editor, onChange, vaultPath, editable = true, locale = 'en' }: {
  editor: ReturnType<typeof useCreateBlockNote>
  onChange?: () => void
  vaultPath?: string
  editable?: boolean
  ...
  // 「最大宽度」由外层 RichEditorSurface 按用户偏好注入到 .editor-scroll-area;此处若再注入
  // theme 默认的 --editor-max-width,会以更近祖先身份遮蔽偏好值,导致设置里的「最大宽度」
  // 对正文(.bn-editor)失效。剔除该键,让偏好沿层叠向下生效。
  const editorCssVars = useMemo(() => {
    const next: Record<string, string> = { ...(cssVars as Record<string, string>) }
    delete next['--editor-max-width']
    return next
  }, [cssVars])
  ...
  useEditorLinkActivation(containerRef, NOOP_NAVIGATE, vaultPath)
  ...
  return (
    <div
      ...
      style={editorCssVars as React.CSSProperties}
