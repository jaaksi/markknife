...
import { createMarkknifeCodeBlockOptions } from './codeBlockOptions'
...
function MathBlockEditor({ block, editor }: MathBlockEditorProps) {
...
function mediaBlockPropsForPreviewRuntime<T extends MediaBlockPreviewProps>(
  props: T,
...
function MarkknifeAudioBlock(props: AudioBlockProps) {
...
function MarkknifeVideoBlock(props: VideoBlockProps) {
  const externalMediaPreview = useExternalMediaPreview()
...
const codeBlock = createCodeBlockSpec(createMarkknifeCodeBlockOptions())
const audioBlock = AudioBlockSpec()
const mathBlock = MathBlock()
const mermaidBlock = MermaidBlock()
const videoBlock = VideoBlockSpec()

// ==高亮== 的 markdown 行内样式:渲染为 <mark class="markdown-highlight">,序列化回 == 语法
