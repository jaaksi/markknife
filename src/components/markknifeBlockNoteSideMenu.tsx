...
type MarkknifeBlockNoteEditor = BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>
type MarkknifeBlock = NonNullable<ReturnType<MarkknifeBlockNoteEditor['getBlock']>>
type SideMenuBlock = {
  content?: unknown
  id: string
  type: string
...
function liveSideMenuBlock(editor: MarkknifeBlockNoteEditor, block: SideMenuBlock | undefined) {
...
function hasChildBlock(block: MarkknifeBlock, blockId: string): boolean {
...
function editorBlockElement(editor: MarkknifeBlockNoteEditor): HTMLElement | null {
...
function createSideMenuAlignmentController(editor: MarkknifeBlockNoteEditor, blockId: string) {
...
function useSideMenuTextAlignment(editor: MarkknifeBlockNoteEditor, block: SideMenuBlock | undefined) {
...
function validDropTarget({
  ...
  editor: MarkknifeBlockNoteEditor
...
function MarkknifeAddBlockButton() {
...
function MarkknifeDragHandleButton({
  children,
  dragHandleMenu,
  ...
  return (
    ...
    >
      <Components.Generic.Menu.Trigger>
        <span
          className="markknife-block-drag-handle"
...
function MarkknifeRemoveBlockItem({ children }: { children: ReactNode }) {
  const Components = useComponentsContext()!
  const { block, editor } = useSideMenuBlock()
...
function MarkknifeTableHeaderItem({
  children,
  header,
}: {
  children: ReactNode
...
function MarkknifeDragHandleMenu() {
  const dict = useDictionary()

  return (
    <DragHandleMenu>
      <MarkknifeRemoveBlockItem>{dict.drag_handle.delete_menuitem}</MarkknifeRemoveBlockItem>
      <MarkknifeTableHeaderItem header="row">{dict.drag_handle.header_row_menuitem}</MarkknifeTableHeaderItem>
      <MarkknifeTableHeaderItem header="column">{dict.drag_handle.header_column_menuitem}</MarkknifeTableHeaderItem>
    </DragHandleMenu>
  )
}

export function MarkknifeSideMenu(props: SideMenuProps) {
  const { block, editor } = useSideMenuBlock()
  useSideMenuTextAlignment(editor, block)

  return (
    <SideMenu {...props}>
      <MarkknifeAddBlockButton />
      <MarkknifeDragHandleButton dragHandleMenu={MarkknifeDragHandleMenu} />
    </SideMenu>
  )
}
