...
import {
  filterMarkknifeFormattingToolbarItems,
  getMarkknifeBlockTypeSelectItems,
} from './markknifeEditorFormattingConfig'
...
type MarkknifeBasicTextStyle = 'bold' | 'italic' | 'strike' | 'code' | typeof MARKDOWN_HIGHLIGHT_STYLE

const FORMATTER_CLOSE_GRACE_MS = 160
const FORMATTER_VIEWPORT_PADDING_PX = 8
type MarkknifeFloatingOptions = NonNullable<FloatingUIOptions['useFloatingOptions']>
type MarkknifeFloatingMiddleware = NonNullable<MarkknifeFloatingOptions['middleware']>[number]
...
const MARKKNIFE_BASIC_TEXT_STYLE_TOOLTIPS = {
  bold: {
    label: 'Bold',
  ...
  MarkknifeBasicTextStyle,
  { label: string; mainTooltip: string; secondaryTooltip: string }
>

const MARKKNIFE_BASIC_TEXT_STYLE_ICONS = {
  bold: Bold,
  italic: Italic,
  strike: Strikethrough,
  code: Code2,
  [MARKDOWN_HIGHLIGHT_STYLE]: Highlighter,
} satisfies Record<MarkknifeBasicTextStyle, PhosphorIcon>

type MarkknifeSelectedBlock = ReturnType<
  BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>['getTextCursorPosition']
>['block']

type MarkknifeSelectedFileBlock = {
  type: string
  url: string
}
...
type MarkknifeBlockTypeSelectOption = ReturnType<
  typeof getMarkknifeBlockTypeSelectItems
>[number] & {
...
function viewportClampMiddleware(): MarkknifeFloatingMiddleware {
  return {
    name: 'markknifeViewportClamp',
...
function withViewportSafeMiddleware(
  options?: MarkknifeFloatingOptions,
): MarkknifeFloatingOptions {
  if (!options) {
    return {
...
function getSelectedBlocksSafely(
  ...
  try {
    return [editor.getTextCursorPosition().block as MarkknifeSelectedBlock]
  } catch {
    return []
  }
}
...
function getBasicTextStyleButtonState(
  basicTextStyle: MarkknifeBasicTextStyle,
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
) {
...
function getBlockTypeItemIconElement(
  item: ReturnType<typeof getMarkknifeBlockTypeSelectItems>[number],
) {
...
function isSelectedBlockTypeItem(
  item: ReturnType<typeof getMarkknifeBlockTypeSelectItems>[number],
  firstSelectedBlock: MarkknifeSelectedBlock,
) {
...
function getMarkknifeBlockTypeSelectOptions(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
  firstSelectedBlock: MarkknifeSelectedBlock,
) {
  return getMarkknifeBlockTypeSelectItems()
    .filter((item) =>
      editorHasBlockWithType(
        editor,
        item.type,
...
function getSelectedFileBlockState(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
): MarkknifeSelectedFileBlock | null {
...
function liveSelectedBlock(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
  block: MarkknifeSelectedBlock,
) {
  try {
    return editor.getBlock(block.id) as MarkknifeSelectedBlock | undefined
  } catch (error) {
...
function liveSelectedBlocks(
  editor: BlockNoteEditor<BlockSchema, InlineContentSchema, StyleSchema>,
  selectedBlocks: MarkknifeSelectedBlock[],
) {
  const liveBlocks: MarkknifeSelectedBlock[] = []
...
function MarkknifeBasicTextStyleButton({
  basicTextStyle,
}: {
  basicTextStyle: MarkknifeBasicTextStyle
}) {
  ...
  const Icon = Reflect.get(MARKKNIFE_BASIC_TEXT_STYLE_ICONS, basicTextStyle) as PhosphorIcon
  const copy = Reflect.get(MARKKNIFE_BASIC_TEXT_STYLE_TOOLTIPS, basicTextStyle) as {
    label: string
    mainTooltip: string
...
function MarkknifeBlockTypeSelect() {
  const editor = useBlockNoteEditor<
    BlockSchema,
    InlineContentSchema,
    StyleSchema
  ...
  const selectedBlocks = useEditorState({
    editor,
    selector: ({ editor }): MarkknifeSelectedBlock[] => getSelectedBlocksSafely(editor),
  })
  const firstSelectedBlock = selectedBlocks[0] ?? null
  const selectItems = useMemo(
    () => (
      firstSelectedBlock
        ? getMarkknifeBlockTypeSelectOptions(editor, firstSelectedBlock)
        : []
    ),
    [editor, firstSelectedBlock],
  )
  const selectedItem = selectItems.find(
    (item): item is MarkknifeBlockTypeSelectOption => item.isSelected,
  )
...
function MarkknifeFileDownloadButton({ vaultPath }: { vaultPath?: string }) {
  const Components = useComponentsContext()!
  ...
  return items.flatMap((item) => {
    switch (String(item.key)) {
      case 'blockTypeSelect':
        return [<MarkknifeBlockTypeSelect key={item.key} />]
      case 'boldStyleButton':
        return [<MarkknifeBasicTextStyleButton basicTextStyle="bold" key={item.key} />]
      case 'italicStyleButton':
        return [<MarkknifeBasicTextStyleButton basicTextStyle="italic" key={item.key} />]
      case 'strikeStyleButton':
        return [<MarkknifeBasicTextStyleButton basicTextStyle="strike" key={item.key} />]
      case 'fileDownloadButton':
        return [<MarkknifeFileDownloadButton key={item.key} vaultPath={vaultPath} />]
      default:
        return [item]
    }
  })
...
function insertExtraTextStyleButtons(items: ReactElement[]) {
  ...
  return [
    ...
    <MarkknifeBasicTextStyleButton basicTextStyle="code" key="codeStyleButton" />,
    <MarkknifeBasicTextStyleButton
...
function getMarkknifeFormattingToolbarItems(vaultPath?: string) {
...
export function MarkknifeFormattingToolbar({ vaultPath }: { vaultPath?: string } = {}) {
  return <FormattingToolbar>{getMarkknifeFormattingToolbarItems(vaultPath)}</FormattingToolbar>
}

export function MarkknifeFormattingToolbarController(props: {
  ...
  const Component = props.formattingToolbar || MarkknifeFormattingToolbar
