...
type MarkknifeSlashMenuItem = DefaultReactSuggestionItem & { key: string }
type MarkknifeBlockTypeSelectItem = {
  name: string
  type: string
...
type MarkknifeSlashMenuLabels = {
  mathTitle: string
}

const MERMAID_SLASH_COMMAND_DIAGRAM = [
  'flowchart TD',
  '    edit["Switch to the raw editor to edit"]',
].join('\n')
const MATH_SLASH_COMMAND_LATEX = '\\sqrt{a^2 + b^2}'
...
const MARKKNIFE_BLOCK_TYPE_SELECT_ITEMS: MarkknifeBlockTypeSelectItem[] = [
...
const MARKKNIFE_SLASH_MENU_ICONS: Partial<Record<string, PhosphorIcon>> = {
  audio: SpeakerHigh,
...
function createMathSlashMenuItem(
  editor: Parameters<typeof getDefaultReactSlashMenuItems>[0],
  labels: MarkknifeSlashMenuLabels = { mathTitle: 'Math' },
): MarkknifeSlashMenuItem {
  return createBlockSlashMenuItem(editor, {
    key: 'math',
...
function createBlockSlashMenuItem(
  ...
  } as MarkknifeSlashMenuItem
}

function addItemsToMediaGroup(
  items: MarkknifeSlashMenuItem[],
  mediaItems: MarkknifeSlashMenuItem[],
): MarkknifeSlashMenuItem[] {
...
function createMarkknifeSlashMenuIcon(Icon: PhosphorIcon) {
  return createElement(
    'span',
    { className: 'markknife-slash-menu-icon' },
    createElement(Icon, {
      'aria-hidden': true,
      className: 'markknife-slash-menu-icon__regular',
      size: 18,
      weight: 'regular',
    }),
    createElement(Icon, {
      'aria-hidden': true,
      className: 'markknife-slash-menu-icon__fill',
      size: 18,
      weight: 'fill',
    }),
  )
...
export function getMarkknifeBlockTypeSelectItems() {
  return MARKKNIFE_BLOCK_TYPE_SELECT_ITEMS
}

export function filterMarkknifeFormattingToolbarItems<T extends ReactElement>(
  items: T[],
): T[] {
  return items.filter(
...
function filterMarkknifeSlashMenuItems<T extends MarkknifeSlashMenuItem>(
  items: T[],
): T[] {
  return items
    .filter((item) => !UNSUPPORTED_SLASH_MENU_KEYS.has(item.key))
    .map((item) => {
      const MarkknifeIcon = MARKKNIFE_SLASH_MENU_ICONS[item.key]

      return {
        ...item,
        icon: MarkknifeIcon ? createMarkknifeSlashMenuIcon(MarkknifeIcon) : item.icon,
        subtext: undefined,
      }
    }) as T[]
}

export function getMarkknifeSlashMenuItems(
  editor: Parameters<typeof getDefaultReactSlashMenuItems>[0],
  query: string,
  labels?: MarkknifeSlashMenuLabels,
) {
  const items = addItemsToMediaGroup(
    getDefaultReactSlashMenuItems(editor) as MarkknifeSlashMenuItem[],
    [
