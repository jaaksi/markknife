...
/** 单文件 Markdown 应用在浏览器/测试环境的内存文件存储（read/write_markdown_file）。 */
const DEV_WELCOME_MARKDOWN = `# Welcome to the Markdown editor

This file is served by the dev mock so the three modes can be exercised in a browser.
...
| Mode | Editor |
| --- | --- |
| View | BlockNote (read-only) |
| WYSIWYG | BlockNote |
| Split | CodeMirror + preview |
`

const mockMarkdownFiles: Record<string, string> = {
  'mock-vault/welcome.md': DEV_WELCOME_MARKDOWN,
}

function readMockMarkdownFile(path: string): string {
  return mockMarkdownFiles[path] ?? MOCK_CONTENT[path] ?? DEV_WELCOME_MARKDOWN
}
...
export const mockHandlers: Record<string, (args: any) => any> = {
  ...
  read_markdown_file: (args: { path: string }) => readMockMarkdownFile(args.path),
  write_markdown_file: (args: { path: string; content: string }) => {
    mockMarkdownFiles[args.path] = args.content
  },
  // 浏览器/测试环境不做真实更新：检查恒返回「无更新」。
  ...
  get_agent_docs_path: () => '/mock/Markknife/resources/agent-docs',
  ...
  get_mcp_config_snippet: () => JSON.stringify({
    mcpServers: {
      markknife: {
        type: 'stdio',
        command: 'node',
        args: ['/mock/Markknife/mcp-server/index.js'],
        env: {
          WS_UI_PORT: '9711',
        },
      },
