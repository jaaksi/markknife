...
type VaultPropertyScalar = string | number | boolean | null
type VaultPropertyArray = Array<string | number | boolean>
type VaultPropertyValue = VaultPropertyScalar | VaultPropertyArray
...
/** Vault-scoped UI configuration stored locally per vault path. */
interface AllNotesConfig {
  noteListProperties: string[] | null
}

/** Vault-scoped UI configuration stored locally per vault path. */
type NoteLayout = 'centered' | 'left'

export type NoteWidthMode = 'normal' | 'wide'
type GitSetupPreference = 'prompt' | 'never'
