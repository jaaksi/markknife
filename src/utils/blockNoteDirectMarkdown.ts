...
export interface DirectMarkdownCapableSerializer {
  ...
  __markknifeDirectMarkdownCache?: WeakMap<object, Map<string, string>>
  __markknifeLastDirectMarkdownMetrics?: BlockNoteDirectMarkdownMetrics
}
...
export function installBlockNoteDirectMarkdown(editor: DirectMarkdownCapableSerializer): void {
  ...
  editor.__markknifeDirectMarkdownCache = cache
  editor.blocksToMarkdownDirect = (blocks: unknown[]) => {
    const result = blocksToMarkdownDirect(blocks, cache)
    editor.__markknifeLastDirectMarkdownMetrics = result.metrics
    return result
  }
}
