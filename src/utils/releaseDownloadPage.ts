const RELEASE_HISTORY_URL = 'https://markknife.md/releases/'
const DOWNLOAD_FRAME_NAME = 'markknife-download-frame'
const WINDOWS_MANAGED_INSTALL_NOTE =
  'Windows installers are Authenticode-signed. Company-managed devices may still require IT to approve the Markknife publisher before first install.'
...
const PLATFORM_METADATA: Record<StablePlatformKey, { buttonLabel: string; label: string }> = {
  'darwin-aarch64': {
    buttonLabel: 'Download Markknife for macOS Apple Silicon',
    label: 'macOS Apple Silicon',
  },
  'darwin-x86_64': {
    buttonLabel: 'Download Markknife for Intel Mac',
    label: 'macOS Intel',
  },
  'linux-x86_64': {
    buttonLabel: 'Download Markknife AppImage for Linux',
    label: 'Linux AppImage',
  },
  'linux-x86_64-rpm': {
    buttonLabel: 'Download Markknife RPM for Linux',
    label: 'Linux RPM',
  },
  'windows-x86_64': {
    buttonLabel: 'Download Markknife for Windows',
    label: 'Windows',
  },
}
...
function extractStableDownloadTargets(payload: unknown): StableDownloadTargets {
...
function extractStableDownloadTargetsFromReleases(
  payload: unknown,
): StableDownloadTargets {
...
function buildStableDownloadPageContent(
  ...
  if (Object.keys(downloads).length > 0) {
    return {
      ...
      message: 'Preparing the latest stable Markknife download for your platform.',
      shouldRedirect: true,
      title: 'Markknife Stable Download',
    }
  }

  return {
    helperText: 'Use the button below to check the latest release history.',
    message: 'No stable Markknife downloads are available yet.',
    shouldRedirect: false,
    title: 'Markknife Stable Download Unavailable',
  }
}
...
function buildDownloadFrameMarkup(downloads: StableDownloadTargets): string {
  ...
  return `<iframe title="Markknife installer download" name="${DOWNLOAD_FRAME_NAME}" sandbox="allow-downloads" hidden></iframe>`
}
    ...
    <script>
      ...
      function downloadMessage(target, requiresMacChoice, requiresWindowsChoice) {
        ...
        if (requiresWindowsChoice) {
          return 'Use the signed Windows installer link below. Company-managed devices may require IT approval of the Markknife publisher.';
        }

        return 'Starting the latest stable Markknife download for ' + target.label + '.';
      }
